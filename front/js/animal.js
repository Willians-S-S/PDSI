...
function exibirDadosNaPagina(data, image) {
    ...
    const tabela = document.getElementById("historico-peso").querySelector("tbody");
tabela.innerHTML = ""; // limpa antes de preencher

animal.historys.forEach(element => {
    const linha = document.createElement("tr");

    const pesoPredito = document.createElement("td");
    pesoPredito.textContent = `${element.current_weight.toFixed(2)} kg`;

    const pesoManual = document.createElement("td");
    pesoManual.textContent = element.weight_manual !== null ? `${element.weight_manual.toFixed(2)} kg` : "Não informado";

    const data = document.createElement("td");
    data.textContent = new Date(element.created_at).toLocaleDateString();

    linha.appendChild(pesoPredito);
    linha.appendChild(pesoManual);
